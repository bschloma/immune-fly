...
def filter(arr, _sigma_blur, _sigmas, _alpha, _beta, _gamma, thresh):
    # unpack this zslice
    arr = arr[0]
    ...
    if _sigmas > 0:
        arr = difference_of_gaussians(arr, low_sigma=_sigmas, high_sigma=3*_sigmas)
    # create and apply a mask
    arr = arr > thresh
    #arr = arr * (og_arr > 3000)
    #arr = binary_opening(arr, disk(19))
    #arr = binary_erosion(arr, disk(5))
    # arr = binary_closing(arr, disk(39))
    # arr = remove_small_holes(arr, 25000)
    #arr = arr == 0
...
def gpu_process(darr, sigma_blur=1.0, beta=0.5, l_gamma=-0.3, l_thresh=-12.0, sigmas=8.0):
    ...
    # lazy apply filter
    filt = da.map_blocks(filter, mem_cu, sigma_blur, sigmas, alpha, beta, _gamma=10**l_gamma, thresh=10**l_thresh, dtype=np.float32)
...
path_to_zarr = r'/media/brandon/Data2/Brandon/fly_immune/Lightsheet_Z1/2025_06_10_gfp-rel_ecoli-hs-dtom_6hrs/larva_3/im.ome.zarr/2'
