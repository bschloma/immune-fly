...
def gpu_process(darr, beta=0.5, l_gamma=-0.3, l_thresh=-12.0, sigmas=8.0):
    sigma_blur = 3.0
    #sigmas = 8.0  # np.arange(1, 10, 2)
    alpha = 0.5
    # lazy move to gpu
...
@magicgui(
    ...
    l_thresh={"widget_type": "FloatSlider", "max": 0, "min": -20},
    sigmas={"widget_type": "FloatSlider", "max": 50.0, "min": 1.0},
    layout='vertical'
)
def magic_func(layer: ImageData, beta: float = 0.5, l_gamma: float = -0.3, l_thresh: float = -12.0, sigmas: float = 8.0) -> ImageData:
    if layer is not None:
        return gpu_process(layer, beta, l_gamma, l_thresh, sigmas)
